    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-[680px]">
        ...
        <>
          ...
          <div className="bg-card rounded border border-border overflow-hidden">
            <table className="w-full border-collapse text-[9px]">
              <thead>
                <tr className="bg-secondary text-muted-foreground text-[8px]">
                ...
                {elements.map((el, i) => {
                  ...
                  return (
                    <tr key={el.id} className="border-t border-border">
                      <td className="px-[6px] py-1 text-foreground/80">{i + 1}</td>
                      <td className="px-[6px] py-1 text-foreground/80">{el.label}{el.surface === 'rear' ? ' [R]' : ''}</td>
                      <td className="px-[6px] py-1 text-foreground/80">{f ? 'circle+4\u00d7bolt' : c?.cut || 'rect'}</td>
                      <td className="px-[6px] py-1 text-foreground/80">
                        {f ? `\u2300${f.cutoutDiameter}` : c?.cut === 'round' ? `\u2300${((c.r ?? 0) * 2).toFixed(1)}` : `${el.w}\u00d7${el.h}`}
                      </td>
                      <td className="px-[6px] py-1 text-foreground/80">{el.x.toFixed(1)}</td>
                      <td className="px-[6px] py-1 text-foreground/80">{el.y.toFixed(1)}</td>
                      <td className="px-[6px] py-1 text-foreground/80">{c?.depthBehind || d?.d || f?.depthBehind || '\u2014'}mm</td>
              ...
              <tbody>
                {bom.map((b, i) => (
                  <tr key={i} className="border-t border-border">
                    <td className="px-[6px] py-1 text-primary font-bold">{b.qty}</td>
                    <td className="px-[6px] py-1 text-foreground/80">{b.part}</td>
                    <td className="px-[6px] py-1 text-foreground/80">{b.spec}</td>
                    <td className="px-[6px] py-1 text-muted-foreground">{b.note ?? ''}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {ribs.map((r, i) => (
                  <tr key={i} className="border-t border-border">
                    ...
                    <td className="px-[6px] py-1 text-foreground/80">{r.w.toFixed(1)}{'\u00d7'}{r.h.toFixed(1)}</td>
                    <td className="px-[6px] py-1 text-foreground/80">{r.depth.toFixed(0)}mm</td>
                    <td className="px-[6px] py-1 text-muted-foreground">{r.reason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full border-collapse text-[9px]">
              ...
              <tbody>
                {marginWarnings.map((w, i) => {
                  ...
                  return (
                    <tr key={i} className="border-t border-border">
                      <td className="px-[6px] py-1 text-foreground/80">{el?.label ?? w.elementId.slice(0, 6)}</td>
                      <td className="px-[6px] py-1 text-foreground/80">{w.edge}{w.neighborId ? ` (${elements.find(e => e.id === w.neighborId)?.label ?? '?'})` : ''}</td>
                      <td className="px-[6px] py-1 text-foreground/80">{w.gap.toFixed(1)}mm</td>
                      <td className="px-[6px] py-1 text-foreground/80">{w.minGap.toFixed(1)}mm</td>
      ...
      {assemblyMode === 'modular' && trayReinforcements.length > 0 && (
        <>
          ...
          {/* Weight budget */}
          <div className="bg-card rounded border border-border p-[10px] mb-[8px]">
            ...
            <div className="h-[4px] bg-input rounded-sm overflow-hidden">
              <div
          ...
          {trayReinforcements.map(tr => (
            <div key={tr.elementId} className="bg-card rounded border border-border p-[10px] mb-[6px]">
              <div className="text-[9px] text-foreground/80 font-semibold mb-1">Tray: {tr.label}</div>
              <SpecTable rows={[
